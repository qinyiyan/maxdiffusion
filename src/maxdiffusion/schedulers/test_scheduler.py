...
def simulate_unipc_scheduler(
    ...
    print(f"Sample shape: {sample_shape}, Inference steps: {num_inference_steps}, Seed: {seed}, Device: {device}")
    ...
    sample = torch.randn(sample_shape, generator=generator, dtype=torch.float32).to(device)
    ...
    for i, t in enumerate(scheduler.timesteps):
        ...
        model_output = torch.randn(sample_shape, generator=generator, dtype=torch.float32).to(device)
    ...
    print(f"Final sample shape: {sample.shape}, dtype: {sample.dtype}, device: {sample.device}")
