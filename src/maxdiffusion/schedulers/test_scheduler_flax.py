...
def simulate_unipc_scheduler_jax(
    ...
    print(f"Sample shape: {sample_shape}, Inference steps: {num_inference_steps}, Seed: {seed}")
    
    output_dir = "/tmp/jax_steps/"

    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    print(f"Intermediate denoising steps will be saved to: {os.path.abspath(output_dir)}")
    ...
    flax_state = flax_scheduler.set_timesteps(flax_state, num_inference_steps, sample_shape)
    print("\nScheduler initialized.")
    print(f"  flax_state timesteps shape: {flax_state.timesteps}")
    ...
    jnp.save(initial_filename, sample.copy()) # .copy() ensures it's on CPU and is a numpy array
    ...
    for i, t in enumerate(flax_state.timesteps):
        ...
        jnp.save(output_filename, sample.copy()) # .copy() ensures it's a NumPy array (on CPU) for saving
...
if __name__ == "__main__":
    ...
    latent_tensor_shape = (1, 4, 64, 64) # Example latent tensor shape (Batch, Channels, Height, Width)
