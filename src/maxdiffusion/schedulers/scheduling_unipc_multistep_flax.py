...
class FlaxUniPCMultistepScheduler(FlaxSchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        self, state: UniPCMultistepSchedulerState, num_inference_steps: int, shape: Tuple
        ...
        model_outputs = jnp.zeros(
            (self.config.solver_order,) + shape,
            dtype=self.dtype
        )
        timestep_list = jnp.zeros(
            (self.config.solver_order,),
            dtype=jnp.int32 # Timesteps are integers
        )
    ...
    def multistep_uni_p_bh_update(
        ...
        sigma_t_val, sigma_s0_val = state.sigmas[state.step_index + 1], state.sigmas[state.step_index]
    ...
    def multistep_uni_c_bh_update(
        self,
        state: UniPCMultistepSchedulerState,
        this_model_output: jnp.ndarray, # Model output for current timestep `t`
    ...
    def index_for_timestep(self, state: UniPCMultistepSchedulerState, timestep: Union[int, jnp.ndarray], schedule_timesteps: Optional[jnp.ndarray] = None) -> int:
        """JAX version of `index_for_timestep`."""
    ...
    def _init_step_index(self, state: UniPCMultistepSchedulerState, timestep: Union[int, jnp.ndarray]) -> UniPCMultistepSchedulerState:
    ...
    def step(
        ...
        timestep_scalar = timestep.item() if isinstance(timestep, jnp.ndarray) and timestep.ndim == 0 else int(timestep) # Ensure int type
        ...
        # Ensure history buffer is large enough; should be ensured by `create_state`
        # If current step is 0 (first step), just set it directly without rolling.

        ### TODO: This order is different from Huggingface!!!!!
        # HF: correct first, and then roll!!
        ...
        if use_corrector:
            corrected_sample = self.multistep_uni_c_bh_update(
                state=state,
                this_model_output=model_output_for_history, # Corrected model output from history
                last_sample=state.last_sample, # Sample after previous predictor step
                this_sample=sample, # Current sample before corrector
                order=state.this_order, # Current order for correction
            )
            sample = corrected_sample # Update sample with corrected value
    ...
    def add_noise(
        ...
        """
        if timesteps.ndim > 0:
            raise ValueError(f"For `add_noise`, `timesteps` must be a scalar (0-dim JAX array), but got shape {timesteps.shape}.")
        
        # Get `step_indices` for the current timestep.
        if state.begin_index is None:
            step_idx = self.index_for_timestep(state, timesteps.item(), state.timesteps)
            step_indices = jnp.array([step_idx]) # Make it a 1-element array for indexing sigmas
